...
const precioTotalFord = marcaFord.reduce((previousValue, coche) => {
  return previousValue + coche.precio;
}, 0);

console.log(precioTotalFord);

const precioMedia = precioTotalFord / marcaFord.length;
console.log(precioMedia);

//PUNTO CUATRO 4. Obtén un array con las distintas marcas de coches (no se pueden repetir).

const marcas = coches.map((coche) => {
  return coche.marca;
});

const marcasSinDuplicados = [...new Set(marcas)]; //aqui hacemos lo del corchete y tres puntitos para que el new set se guarde en un array, y si no hacemos eso, solo nos daria una string
